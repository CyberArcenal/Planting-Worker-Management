 ...
 */
module.exports = class InitSchema1772697708376 {
    name = 'InitSchema1772697708376'
    ...
    async up(queryRunner) {
        ...
        await queryRunner.query(`CREATE TABLE "bukids" ("id" integer PRIMARY KEY AUTOINCREMENT NOT NULL, "name" varchar NOT NULL, "status" varchar CHECK( "status" IN ('initiated','active','complete','inactive') ) NOT NULL DEFAULT ('inactive'), "notes" varchar, "location" varchar, "createdAt" datetime NOT NULL DEFAULT (datetime('now')), "updatedAt" datetime NOT NULL DEFAULT (datetime('now')), "sessionId" integer)`);
        ...
        await queryRunner.query(`CREATE TABLE "workers" ("id" integer PRIMARY KEY AUTOINCREMENT NOT NULL, "name" varchar NOT NULL, "contact" varchar, "email" varchar, "address" varchar, "status" varchar NOT NULL DEFAULT ('active'), "hireDate" datetime, "createdAt" datetime NOT NULL DEFAULT (datetime('now')), "updatedAt" datetime NOT NULL DEFAULT (datetime('now')), CONSTRAINT "UQ_87f2092ffaae628ef63547d2442" UNIQUE ("email"))`);
        ...
        await queryRunner.query(`CREATE INDEX "IDX_ASSIGNMENT_DATE" ON "assignments" ("assignmentDate") `);
        await queryRunner.query(`CREATE INDEX "IDX_ASSIGNMENT_STATUS" ON "assignments" ("status") `);
        await queryRunner.query(`CREATE TABLE "temporary_bukids" ("id" integer PRIMARY KEY AUTOINCREMENT NOT NULL, "name" varchar NOT NULL, "status" varchar CHECK( "status" IN ('initiated','active','complete','inactive') ) NOT NULL DEFAULT ('inactive'), "notes" varchar, "location" varchar, "createdAt" datetime NOT NULL DEFAULT (datetime('now')), "updatedAt" datetime NOT NULL DEFAULT (datetime('now')), "sessionId" integer, CONSTRAINT "FK_70b48b86b0daf745c9181d3d1a6" FOREIGN KEY ("sessionId") REFERENCES "sessions" ("id") ON DELETE CASCADE ON UPDATE NO ACTION)`);
