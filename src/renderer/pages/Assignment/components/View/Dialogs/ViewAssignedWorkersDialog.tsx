...
const ViewAssignedWorkersDialog: React.FC<ViewAssignedWorkersDialogProps> = ({
  ...
  const fetchAssignedWorkers = async () => {
    ...
    try {
      // Use getAll with pitakId filter
      const response = await assignmentAPI.getAll({ pitakId });

      if (response.status && response.data) {
        const assignments = response.data; // array of Assignment
          ...
          if (assignment.worker) {
            ...
            if (!workerMap.has(workerId)) {
              workerMap.set(workerId, {
                ...
                code: assignment.worker.id.toLocaleString(),
                contact: assignment.worker.contact as string,
                totalAssignments: 0,
                totalLuWang: 0,
                activeAssignments: 0,
  ...
  const confirmRemoveWorker = async () => {
    ...
    try {
      // Get all active assignments for this worker in this pitak
      ...
      // Update each active assignment to cancelled status using update()
      const updatePromises = activeAssignments.map((assignment) =>
        assignmentAPI.update(assignment.id, {
          status: "cancelled",
          notes: `Worker ${workerName} (${workerId}) removed from pitak by user`,
        })
      );
  ...
  const filteredWorkers = workers.filter((worker) => {
    ...
    if (searchTerm) {
      ...
      return (
        ...
        (worker.contact && worker.contact.includes(searchTerm))
      );
    }
  ...
  const handleExport = async () => {
    try {
      // Fetch all assignments for this pitak (we could reuse workers, but to be safe we fetch fresh)
      const response = await assignmentAPI.getAll({ pitakId });
      if (!response.status || !response.data) {
      ...
      // Generate CSV
      const headers = ["ID", "Date", "Worker", "LuWang", "Status", "Notes"];
      ...
      a.href = url;
      a.download = `pitak-${pitakId}-assignments-${new Date().toISOString().slice(0, 10)}.csv`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      URL.revokeObjectURL(url);

      dialogs.success(`Exported ${assignments.length} records to CSV.`);
    } catch (err: any) {
