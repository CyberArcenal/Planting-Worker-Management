// src/components/Dialogs/AddAssignmentNoteDialog.tsx
import React from 'react';
import { X, FileText, Save, Loader2, Tag } from 'lucide-react';
import type { useAddAssignmentNote } from './hooks/useAddAssignmentNote';


interface AddAssignmentNoteDialogProps {
  hook: ReturnType<typeof useAddAssignmentNote>;
}
...
const AddAssignmentNoteDialog: React.FC<AddAssignmentNoteDialogProps> = ({ hook }) => {
  const {
    note,
    setNote,
    noteType,
  ...
  const currentNoteType = noteTypeOptions.find(opt => opt.value === noteType) || noteTypeOptions[0];
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-[100] p-4">
      <div className="bg-white rounded-lg w-full max-w-lg shadow-xl border border-gray-200">
        ...
        <div className="p-4 border-b border-gray-200 flex items-center justify-between bg-gradient-to-r from-green-50 to-white">
          ...
          <button onClick={onClose} className="w-7 h-7 rounded flex items-center justify-center hover:bg-gray-200">
        ...
        <form onSubmit={handleSubmit} className="p-5">
          ...
          <div className="mb-4">
            <label className="block text-sm font-medium text-gray-700 mb-2 flex items-center gap-2">
              <Tag className="w-4 h-4" /> Note Type
            </label>
            ...
            <div className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-md ${currentNoteType.color}`}>
              <span className="text-xs font-medium">Selected: {currentNoteType.label}</span>
            </div>
          </div>
          ...
          <div className="mb-4">
            ...
            <textarea
              ...
              placeholder={`Enter your ${currentNoteType.label.toLowerCase()} here...`}
              disabled={loading}
              maxLength={2000}
            />
            <div className="flex justify-between items-center mt-1">
              <div className="text-xs text-gray-500">Characters: {note.length}/2000</div>
              <div className="text-xs text-gray-500">Lines: {note.split('\n').length}</div>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between pt-4 border-t border-gray-200">
            <div className="text-xs text-gray-500">Tip: Be specific and include relevant details</div>
            <div className="flex items-center gap-2">
              <button
                type="button"
                onClick={onClose}
                disabled={loading}
                className="px-3 py-1.5 rounded text-sm font-medium border border-gray-300 hover:bg-gray-100 text-gray-700 disabled:opacity-50"
              >
                Cancel
              </button>
              <button
                ...
                className="px-4 py-1.5 rounded text-sm font-medium bg-green-600 hover:bg-green-700 text-white disabled:opacity-50 flex items-center gap-2"
              >
                {loading ? (
                  <>
