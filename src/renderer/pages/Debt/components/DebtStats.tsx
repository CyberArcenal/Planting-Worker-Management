...
import type { DebtStats as DebtStatsType } from "../../../api/core/debt";
...
const DebtStats: React.FC<DebtStatsProps> = ({ stats }) => {
  // Compute per‑status counts from statusBreakdown
  const pendingCount = stats?.statusBreakdown?.pending || 0;
  const partiallyPaidCount = stats?.statusBreakdown?.partially_paid || 0;
  const paidCount = stats?.statusBreakdown?.paid || 0;
  const cancelledCount = stats?.statusBreakdown?.cancelled || 0;
  const overdueCount = stats?.statusBreakdown?.overdue || 0;
