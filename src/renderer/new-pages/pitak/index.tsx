// src/renderer/pages/pitak/index.tsx
import React, { useState } from "react";
import { Plus, Download, Filter, RefreshCw, MapPin } from "lucide-react";
...
const PitakPage: React.FC = () => {
  ...
  const handleDelete = async (pitak: any) => {
    const confirmed = await dialogs.confirm({
      title: "Delete Pitak",
      message: `Are you sure you want to delete pitak at "${pitak.location}"?`,
    });
    if (!confirmed) return;
    try {
      await pitakAPI.delete(pitak.id);
  ...
  const handleBulkDelete = async () => {
    ...
    const confirmed = await dialogs.confirm({
      title: "Bulk Delete",
      message: `Delete ${selectedPitaks.length} pitaks?`,
    });
    if (!confirmed) return;
    try {
      await Promise.all(selectedPitaks.map((id) => pitakAPI.delete(id)));
      showSuccess(`${selectedPitaks.length} pitaks deleted.`);
      setSelectedPitaks([]);
      reload();
    } catch (err: any) {
  ...
  const handleExport = async () => {
    ...
    const confirmed = await dialogs.confirm({
      title: "Export Pitaks",
      message: `Export ${pagination.total} pitaks as ${exportFormat.toUpperCase()}?`,
    });
    if (!confirmed) return;
    ...
    >
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-sm mb-4">
        <div>
          ...
          >
            Manage individual plots (pitak) within bukids
          </p>
        </div>
        <div className="flex flex-wrap gap-xs w-full sm:w-auto">
          ...
          >
            ...
            <Button
              ...
              disabled={exportLoading || allPitaks.length === 0}
      ...
      {!loading && !error && (
        <>
          <PitaksTable
            ...
            onToggleSelect={togglePitakSelection}
            ...
            >
              ...
              >
                {Object.values(filters).some(
                  ...
                  : "Start by adding your first pitak"}
              </p>
