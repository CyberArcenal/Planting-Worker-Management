// src/renderer/pages/pitak/components/PitakFormDialog.tsx
...
interface PitakFormDialogProps {
  ...
  pitakId?: number | null;
  bukidId?:number | null;
  initialData?: Partial<Pitak> | null;
  onClose: () => void;
  onSuccess?: (pitak: Pitak) => void;
}
...
const PitakFormDialog: React.FC<PitakFormDialogProps> = ({
  ...
  // Initialize hooks
  const { validateShapeInputs, clearError, clearAllErrors } = useMeasurementValidation();

  // Load bukids when dialog opens
  useEffect(() => {
    if (isOpen) {
      const fetchBukids = async () => {
        try {
          ...
          if (response.status) {
            setBukids(response.data);
          }
        } catch (error) {
          console.error("Error fetching bukids:", error);
        }
      };
      fetchBukids();
    }
  ...
  useEffect(() => {
    ...
    const initializeForm = async () => {
      setLoading(true);
      try {
        if (mode === "edit" && pitakId) {
          // If initialData is provided, use it directly
          if (initialData) {
            const parsed = await parseExistingPitakData(initialData as Pitak);
            setFormData(parsed);
            setCalculationResults({
              areaSqm: parsed.areaSqm,
              totalLuwang: parsed.totalLuwang,
            });
            setLoading(false);
          } else {
            // Otherwise fetch from API
            const response = await pitakAPI.getById(pitakId);
            if (response.status) {
              const pitakData = response.data;
              setPitak(pitakData);
              const parsed = await parseExistingPitakData(pitakData);
              setFormData(parsed);
              setCalculationResults({
    ...
    initializeForm();
  }, [isOpen, mode, pitakId, initialData, onClose]);

  // Helper: Parse existing pitak data
  const parseExistingPitakData = async (pitakData: Pitak): Promise<FormData> => {
    let buholInputs = {};
    ...
    return {
      bukidId: pitakData.bukid?.id || null,
      location: pitakData.location || "",
      totalLuwang: pitakData.totalLuwang,
      areaSqm: pitakData.areaSqm || 0,
      status: pitakData.status as any || "active",
  ...
  const handleAdvancedCalculation = (results: CalculationResults) => {
    ...
    setFormData((prev) => ({
      ...
      measurementMethod: results.totalLuwang > 0 ? "traditional_calculation" : "",
    }));
  };
  ...
  console.log("Validation errors:", errors);
    ...
    try {
      ...
      if (formData.useAdvancedMode) {
        ...
        pitakData.notes = formData.notes.trim() || undefined;
      } else {
        pitakData.notes = formData.notes.trim() || undefined;
      }
      ...
      let response;
      if (mode === "add") {
        response = await pitakAPI.create(pitakData);
      } else {
        if (!pitakId) throw new Error("Pitak ID missing");
        response = await pitakAPI.update(pitakId, pitakData);
      }
                  ...
                  <div className="space-y-6">
                      ...
                      <div className="space-y-4">
                        <div>
                          ...
                          <div className="relative">
                            <BukidSelect
                              value={formData.bukidId || 0}
                              disabled={mode === "edit" || !!bukidId} // Prevent changing bukid in edit mode
                              onChange={async (selectedBukidId: number | null) => {
                                if (selectedBukidId) {
                                  handleChange("bukidId", selectedBukidId);
                                }
                              }}
                    ...
                    <div>
                      ...
                      <MeasurementModeToggle
                        ...
                        onChange={(mode) => handleChange("useAdvancedMode", mode)}
                      />
